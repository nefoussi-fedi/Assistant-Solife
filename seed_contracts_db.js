// Script MongoDB mongosh pour initialiser la base 'solife_contracts'
const dbInstance = db.getSiblingDB("solife_contracts");

print("🧹 Nettoyage des collections de 'solife_contracts'...");
const collections = [
  "contracts", "coverages", "investment_services", "avenants",
  "beneficiaires", "commissions", "bills", "transactions",
  "produits", "fonds", "tarifs", "taxes"
];
collections.forEach(col => dbInstance[col].drop());

// 1. PRODUITS
dbInstance.produits.insertMany([
  {
...
dbInstance.contracts.insertMany([
  ...
  },
  {
    ...
    "fiscalite": {"regime_fiscal": "PER Individuel", "tax_code": "TAX-FR-PER-2026", "date_dernier_rachat": null, "abattement_applicable": false, "duree_detention_fiscale_annees": 2}
  },
  {
    ...
    "versements_programmes": {"mode": "Versements libres", "montant_mensuel": 0, "jour_prelevement": null, "prochain_prelevement": null, "total_versements_annee_en_cours": 10000.00, "date_dernier_versement": "2026-03-15"},
    ...
    "fiscalite": {"regime_fiscal": "Assurance-Vie Droit Français", "tax_code": "TAX-FR-AV-2026", "date_dernier_rachat": null, "abattement_applicable": false, "duree_detention_fiscale_annees": 3}
  },
  {
    ...
    "options_gestion": {"rebalancing_automatique": false, "frequence_rebalancing": "Aucune", "securisation_plus_values": false, "garantie_plancher_deces": true, "stop_loss": false, "dynamisation_interets": false},
    "fiscalite": {"regime_fiscal": "Assurance-Vie Droit Français", "tax_code": "TAX-FR-AV-2026", "date_dernier_rachat": null, "abattement_applicable": false, "duree_detention_fiscale_annees": 1}
  }
]);

// 6. COUVERTURES
dbInstance.coverages.insertMany([
...
// 7. SERVICES D'INVESTISSEMENT
dbInstance.investment_services.insertMany([
  {
...
// 8. BÉNÉFICIAIRES
dbInstance.beneficiaires.insertMany([
...
// 9. AVENANTS
dbInstance.avenants.insertMany([
...
// 10. COMMISSIONS
dbInstance.commissions.insertMany([
...
months.forEach(m => {
  bills.push({
    "bill_id": `BILL-7710-${m}`, "contract_number": "SOL-2022-7710", "type": "Prélèvement mensuel programmé",
    "montant_ttc": 300.00, "devise": "EUR", "date_emission": `${m}-01`, "date_echeance": `${m}-05`,
    "date_paiement": `${m}-05`, "statut": "Payée", "mode_paiement": "Prélèvement SEPA"
  });
});
bills.push({
  "bill_id": "BILL-7710-2026-09", "contract_number": "SOL-2022-7710", "type": "Prélèvement mensuel programmé",
  "montant_ttc": 300.00, "devise": "EUR", "date_emission": "2026-09-01", "date_echeance": "2026-09-05",
  "date_paiement": null, "statut": "En attente", "mode_paiement": "Prélèvement SEPA"
});
bills.push({"bill_id": "BILL-7710-2026-FG", "contract_number": "SOL-2022-7710", "type": "Frais de gestion annuels", "montant_ttc": 640.50, "devise": "EUR", "date_emission": "2026-04-10", "date_paiement": "2026-04-10", "statut": "Prélevée sur encours", "mode_paiement": "Déduction automatique sur le contrat"});
bills.push({"bill_id": "BILL-7710-2026-GD", "contract_number": "SOL-2022-7710", "type": "Prime garantie décès plancher", "montant_ttc": 86.40, "devise": "EUR", "date_emission": "2026-04-10", "date_paiement": "2026-04-10", "statut": "Prélevée sur encours", "mode_paiement": "Déduction automatique sur le contrat"});

["2026-06", "2026-07", "2026-08"].forEach(m => {
  bills.push({"bill_id": `BILL-3320-${m}`, "contract_number": "SOL-2024-3320", "type": "Prélèvement mensuel programmé", "montant_ttc": 200.00, "devise": "EUR", "date_emission": `${m}-01`, "date_echeance": `${m}-10`, "date_paiement": `${m}-10`, "statut": "Payée", "mode_paiement": "Prélèvement SEPA"});
  bills.push({"bill_id": `BILL-1190-${m}`, "contract_number": "SOL-2025-1190", "type": "Prélèvement mensuel programmé", "montant_ttc": 150.00, "devise": "EUR", "date_emission": `${m}-01`, "date_echeance": `${m}-01`, "date_paiement": `${m}-01`, "statut": "Payée", "mode_paiement": "Prélèvement SEPA"});
});
bills.push({"bill_id": "BILL-1190-2026-CD", "contract_number": "SOL-2025-1190", "type": "Prime capital décès toutes causes", "montant_ttc": 420.00, "devise": "EUR", "date_emission": "2026-02-01", "date_paiement": "2026-02-01", "statut": "Prélevée sur encours", "mode_paiement": "Déduction automatique"});
dbInstance.bills.insertMany(bills);

// 12. TRANSACTIONS
dbInstance.transactions.insertMany([
...
// INDEXES
dbInstance.contracts.createIndex({"contract_number": 1}, {unique: true});
dbInstance.contracts.createIndex({"parties_prenantes.assure.party_id": 1});
dbInstance.coverages.createIndex({"contract_number": 1});
dbInstance.investment_services.createIndex({"contract_number": 1});
dbInstance.avenants.createIndex({"contract_number": 1});
dbInstance.beneficiaires.createIndex({"contract_number": 1});
dbInstance.commissions.createIndex({"contract_number": 1});
dbInstance.bills.createIndex({"contract_number": 1});
dbInstance.transactions.createIndex({"contract_number": 1});

print("✅ Base solife_contracts initialisée avec succès !");
